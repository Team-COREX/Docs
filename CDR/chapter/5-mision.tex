...
\section{Objetivos Misión Secundaria}
  ...
  \subsection{Procedimientos}
    ...
    Para cultivarlos \cite{Lauwaet2007-tes} axenicamente se utiliza medio TYI-S-33 (pH 7) suplementado con 10\% de
  ...
  \subsection{Descripción de Subsistemas}
    ...
    \begin{itemize}
      ...
      \item \textbf{On-board Computer (\acrshort{obc}):} Tiene como función principal la gestión autónoma de
        ...
        El propósito fundamental del \acrshort{obc} es leer y almacenar el registro de los sensores,
        ...
        térmica utilizado para la misión secundaria, mediante sensores de temperatura y celdas Peltier, siendo un
        intermediario entre éstas y el sistema de administración de energía (\acrshort{pms}). Además, permite el
        monitoreo en tiempo real de la integridad energética de los distintos subsistemas al ser también el intermediario
        entre el \acrshort{pms} y el \acrshort{hmi}.
        ...
        Será posible seleccionar distintos modos de operación para el sistema mediante el \acrshort{obc}. Esto permite
        una gestión eficiente de los recursos, especialmente en lo que respecta al consumo de potencia y la preparación
        del cubesat para eventos críticos de la misión, mediante la adaptación de su funcionamiento según la etapa de
        la misión. Se han definido los siguiente modos de operación:
        \begin{itemize}
           \item \textbf{Apagado}
           \item \textbf{Pre Lanzamiento:} Inicializa los subsistemas y corrobora el correcto funcionamiento.
            Permite la conexión con la estación terrena para su verificación. Una vez inicializados los subsistemas
            y realizadas las verificiaciones pertinentes, el sistema se pone en modo de reposo.
           \item \textbf{Reposo:} Las unidades de procesamiento principal (RPI en la OBC y Arduino Nano en el PMS) se
           establecen en modos de bajo consumo de potencia (idle, stand-by, deep sleep, etc.). Se disminuye la
           frecuencia de lectura de ciertos sensores. Si el modo anterior fue el de \textit{pre lanzamiento} el
           subsistema \acrshort{tru} se mantiene activo.
        ...
        La selección del modo de operación se implementa mediante un sistema simple y robusto de entrada digital basado
        en la detección de estados lógicos en pines de la Raspberry Pi. Para ello, se ha incluido en la placa un conector
        JST HX de 6 pines (macho), de los cuales 3 pines están conectados a entradas digitales de la Raspberry Pi 
        configuradas internamente con resistencias pull-up. Esto significa que, en ausencia de conexión externa, dichas
        entradas leen un nivel lógico alto (1).

        El sistema de selección se realiza mediante la conexión de diferentes fichas JST HX hembra, diseñadas con
        puentes internos que conectan ciertos pines a masa (GND). De esta forma, se genera una combinación específica
        de niveles lógicos en los pines de entrada de la Raspberry Pi al insertar cada ficha, determinando el modo de
        operación activo. Cada combinación binaria representa un modo de operación previamente definido y programado en
        el firmware del sistema. A continuación se presenta un diagrama que representa esto:
    ...
    \end{figure}
        ...
        \item \textbf{Human-Machine Interface \acrshort{hmi}:} La principal forma de interacción entre el usuario y el
        cubesat es a través de una interfaz web mediante protocolo HTTP, accesible mediante una
        red local generada por la \acrshort{obc}. La segunda forma de interacción es mediante el selector de modos con
        las llaves o conectores de hardware explicado anteriomente. En la Figura \ref{fig:hmi-arq} se puede ver la
        arquitectura de la página mencionada.
