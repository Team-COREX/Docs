...
\section{Descripción del CubeSat}
  ...
  \subsubsection{Power Managment System (\acrshort{pms})}
    ...
    Revise la sección \ref{anexo:pms.schematic} para ver el esquemático del \acrfull{pms}.
  ...
  \subsubsection{On-board Computer (\acrshort{obc})}
   ...
   El archivo CSV resultante cumple dos funciones clave:
    ...
    \subsubsection{Human-Machine Interface \acrshort{hmi}}
      ...
      conectada a la parte física del \acrshort{hmi}. Esta interfaz web, es accesible gracias al punto de acceso WiFi
  ...
  \subsection{Descripción del equipo de soporte en tierra}
    ...
    de forma diferida, los datos recolectados por el CubeSat una vez finalizado el vuelo.  Esta herramienta facilita la
    exploración visual de las variables físicas medidas por los sensores a bordo, representándolas mediante gráficos en
    función del tiempo.  Adicionalmente, incorpora un módulo interactivo en 3D del satélite, que permite manipular la
    línea temporal de la misión, observar el estado de las variables en cada instante y reproducir la animación completa
    del vuelo.
    ...
    la computadora a bordo.  Esta última se encarga de servir los datos almacenados en formato CSV y de proveer los
    archivos de la interfaz web, que se ejecuta en el navegador tras acceder a la URL local hosteada por la Raspberry Pi
    Zero 2~W.  La conexión se establece dentro del rango de la red Wi-Fi generada por la Raspberry, sin requerir acceso
    a internet.
    ...
    Puede revisar la seccion \ref{anexo:webpage} para tener un vistazo de \emph{SatView}.
    ...
    \subsubsection{Procedimiento de descarga}
      La recuperación de datos se llevará a cabo una vez recuperado físicamente el CubeSat mediante conexión directa. El
      CubeSat dispone de un modo de recuperación de datos, el cual se activa insertando la llave adecuada en el puerto
      físico del \acrshort{hmi}. Una vez realizado, la \acrshort{obc} creara una red WiFi. Siguiendo el procedimiento de
      acceso, una vez en la interfaz de \emph{SatView}, se presentaran una serie de botones para descargar los datos de
      la misión.
...
\section{Plan de Proyecto}
  ...
  confiabilidad y compatibilidad con los criterios normativos establecidos.
   ...
   \subsection{Estado de insumos}
    Actualmente hemos adquirido la Raspberry Pi Zero 2 W, que es uno de los componentes centrales y uno de los mas
    propensos a dejar de estar disponible y el filamento de Polipropileno. Se presenta una lista de los elementos que
    aun no se han adquirido pero siguen estando disponibles:
    \begin{itemize}
    ...
    Algunos elementos no listados ya fueron adquiridos para el prototipo, pero sera necesario comprarlos nuevamente
    (chapa de aluminio, varillas, PCB Virgen, etc). Apenas tengamos la confirmación de clasificación, se comenzara con
